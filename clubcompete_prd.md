...
- Only one competition can be active at a time. Creating, editing or restoring a competition is refused if its dates overlap another competition's; a missing open date means "from now" and a missing close date means "never closes". An open competition can be closed early with **Close Competition**.
