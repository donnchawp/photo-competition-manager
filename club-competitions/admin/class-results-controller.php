...
class Results_Controller {
	...
	public function handle_actions(): void {
		...
		if ( 'recalculate_scores' === $action ) {
			...
			check_admin_referer( 'club_competitions_recalculate_scores_' . $competition_id );
		...
		if ( 'export_results_csv' === $action ) {
			...
			check_admin_referer( 'club_competitions_export_results_' . $competition_id );
	...
	public function render(): void {
		...
		if ( empty( $competitions ) ) {
			echo '<div class="wrap">';
			echo '<h1>' . esc_html__( 'Results Dashboard', 'club-competitions' ) . '</h1>';
			echo '<p>' . esc_html__( 'No competitions found. Create a competition first.', 'club-competitions' ) . '</p>';
			echo '</div>';
			return;
		}
		...
		if ( ! $competition ) {
			echo '<div class="wrap">';
			echo '<h1>' . esc_html__( 'Results Dashboard', 'club-competitions' ) . '</h1>';
			echo '<p>' . esc_html__( 'Competition not found.', 'club-competitions' ) . '</p>';
			echo '</div>';
			return;
		}
	...
	private function render_overview( object $competition, array $competitions ): void {
		...
		echo '<label for="competition-select" style="margin-right: 10px;"><strong>' . esc_html__( 'Competition:', 'club-competitions' ) . '</strong></label>';
		...
		if ( ! empty( $selected_category ) ) {
			...
			echo '<div class="club-compete-category-stats" style="background: #f9f9f9; padding: 15px; margin: 20px 0; border-left: 4px solid #2271b1;">';
			echo '<h3>' . esc_html__( 'Category Statistics', 'club-competitions' ) . '</h3>';
			echo '<p>';
		...
		// Action buttons.
		echo '<div class="club-compete-actions" style="margin: 20px 0;">';
		...
		$recalculate_url = wp_nonce_url(
			...
			'club_competitions_recalculate_scores_' . (int) $competition->id
		);
		...
		echo esc_html__( 'Recalculate Scores', 'club-competitions' );
		echo '</a> ';
		...
		$export_url = wp_nonce_url(
			...
			'club_competitions_export_results_' . (int) $competition->id
		);
	...
	private function render_summary_card( string $label, $value, string $icon ): void {
		echo '<div class="club-compete-summary-card" style="background: white; padding: 20px; border: 1px solid #ccd0d4; border-radius: 4px;">';
	...
	private function render_results_table( int $competition_id, string $category, array $grades ): void {
		...
		foreach ( $grades as $grade ) {
			...
			echo '<thead>';
			echo '<tr>';
			echo '<th style="width: 60px;">' . esc_html__( 'Rank', 'club-competitions' ) . '</th>';
			echo '<th style="width: 80px;">' . esc_html__( 'Image', 'club-competitions' ) . '</th>';
			echo '<th>' . esc_html__( 'Member', 'club-competitions' ) . '</th>';
			echo '<th style="width: 100px;">' . esc_html__( 'Score', 'club-competitions' ) . '</th>';
			echo '<th style="width: 80px;">' . esc_html__( 'Votes', 'club-competitions' ) . '</th>';
			echo '<th style="width: 120px;">' . esc_html__( 'Actions', 'club-competitions' ) . '</th>';
			echo '</tr>';
			echo '</thead>';
			echo '<tbody>';
			...
			foreach ( $grade_results as $result ) {
				...
				echo '<td>';
				if ( $image_url ) {
					echo '<img src="' . esc_url( $image_url ) . '" alt="' . esc_attr__( 'Image', 'club-competitions' ) . '" style="max-width: 60px; height: auto; border: 1px solid #ddd;">';
				} else {
				...
				if ( $member ) {
					...
					echo '<em>' . esc_html__( 'Unknown', 'club-competitions' ) . '</em>';
				}
				echo '</td>';
				...
				echo '<a href="' . esc_url( $detail_url ) . '" class="button button-small">' . esc_html__( 'View Details', 'club-competitions' ) . '</a>';
				echo '</td>';
	...
	private function render_image_details( int $image_id, object $competition ): void {
		...
		if ( ! $details['image'] ) {
			echo '<div class="wrap">';
			echo '<h1>' . esc_html__( 'Image Details', 'club-competitions' ) . '</h1>';
			echo '<p>' . esc_html__( 'Image not found.', 'club-competitions' ) . '</p>';
			echo '</div>';
			return;
		}
		...
		echo '<p><a href="' . esc_url( $back_url ) . '" class="button">&larr; ' . esc_html__( 'Back to Results', 'club-competitions' ) . '</a></p>';
		...
		if ( $image_url ) {
			echo '<img src="' . esc_url( $image_url ) . '" alt="' . esc_attr__( 'Competition Image', 'club-competitions' ) . '" style="max-width: 100%; height: auto; border: 1px solid #ddd;">';
		}

		echo '<div style="margin-top: 15px; padding: 15px; background: #f9f9f9; border-left: 4px solid #2271b1;">';
		echo '<p><strong>' . esc_html__( 'Member:', 'club-competitions' ) . '</strong><br>' . esc_html( $member ? $member->name : __( 'Unknown', 'club-competitions' ) ) . '</p>';
		if ( $member && ! empty( $member->email ) ) {
			echo '<p><strong>' . esc_html__( 'Email:', 'club-competitions' ) . '</strong><br>' . esc_html( $member->email ) . '</p>';
		}
		if ( $member ) {
			echo '<p><strong>' . esc_html__( 'Grade:', 'club-competitions' ) . '</strong><br>' . esc_html( $member->grade ) . '</p>';
		}
		echo '<p><strong>' . esc_html__( 'Category:', 'club-competitions' ) . '</strong><br>' . esc_html( $category_label ) . '</p>';
		echo '<p><strong>' . esc_html__( 'Image #:', 'club-competitions' ) . '</strong><br>' . absint( $image->random_number ) . '</p>';
		echo '</div>';
		...
		echo '<h2>' . esc_html__( 'Statistics', 'club-competitions' ) . '</h2>';
		...
		echo '<h2>' . esc_html__( 'Individual Votes', 'club-competitions' ) . '</h2>';
		if ( ! empty( $votes ) ) {
			echo '<table class="wp-list-table widefat fixed striped">';
			echo '<thead>';
			echo '<tr>';
			echo '<th>' . esc_html__( 'Voter', 'club-competitions' ) . '</th>';
			echo '<th>' . esc_html__( 'Score', 'club-competitions' ) . '</th>';
			echo '<th>' . esc_html__( 'Timestamp', 'club-competitions' ) . '</th>';
			echo '</tr>';
			echo '</thead>';
			echo '<tbody>';
			...
			echo '<p>' . esc_html__( 'No votes recorded for this image.', 'club-competitions' ) . '</p>';
		}
	...
	private function export_results_csv( int $competition_id ): void {
		$competition = $this->competitions->find( $competition_id );
		if ( ! $competition ) {
			wp_die( esc_html__( 'Competition not found.', 'club-competitions' ) );
		}
