...
class Email_Results_Job_Manager {
	...
	public function create_job( int $competition_id ) {
		...
		// Store job data.
		update_option( 'club_compete_email_job_' . $job_id, $job_data, false );
	...
	public function schedule_next_batch( string $job_id, int $delay = 0 ): bool {
		...
		return wp_schedule_single_event( $timestamp, 'club_compete_send_results_batch', array( $job_id ) ) !== false;
	}
	...
	public function get_job( string $job_id ): ?array {
		$job_data = get_option( 'club_compete_email_job_' . $job_id, null );
	...
	private function update_job( string $job_id, array $job_data ): bool {
		return update_option( 'club_compete_email_job_' . $job_id, $job_data, false );
	}
	...
	public function cleanup_old_jobs(): int {
		...
		$option_name_pattern = 'club_compete_email_job_%';
