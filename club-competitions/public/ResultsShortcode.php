...
class ResultsShortcode {
	...
	private function render_results( object $competition, bool $hide_names = false ): void {
		...
		?>
		<div class="club-competitions-results">
			<h2><?php echo esc_html( $competition->title ); ?> - <?php esc_html_e( 'Results', 'club-competitions' ); ?></h2>
														...
														<td class="image-cell">
															<?php if ( $thumb_url ) : ?>
																<a href="<?php echo esc_url( $image_urls['full'] ); ?>" target="_blank" rel="noopener noreferrer" class="image-link">
																	<img src="<?php echo esc_url( $thumb_url ); ?>" alt="<?php echo esc_attr( sprintf( __( 'Image %d', 'club-competitions' ), $image->random_number ) ); ?>" loading="lazy" class="result-thumbnail" />
																</a>
															<?php else : ?>
